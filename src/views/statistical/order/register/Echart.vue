...
<template>
  <div class="order-echart-analysis">
    <div class="tab-scroll-container tab-scroll-container-nopagination scrollbarStyle2">
      ...
      <!-- echart柱状图 -->
      <div class="order_echart _marginTopFu" id="registerEchartPrint" v-loading="loading">
        <!-- 标题 -->
        <div
          class="_title"
        >{{searchQuery.startDate |dealTime}}至{{searchQuery.endDate|dealTime}}预约挂号订单趋势分析</div>
        <!-- 内容 -->
        <div class="echart-content statistical-inquiry-echart" ref="echartContent">
          <page-bar-chart class="registerEchart" ref="registerEchart" />
...
export default {
  ...
  methods: {
    ...
    doPrint() {
      var div_print = document.querySelector("#registerEchart"); // 获取要打印部分的内容
      var cv = document.querySelector(".registerEchart canvas"); //获取canvas
      ...
      img.onload = () => {
        ...
        openPrint(this, "#registerEchartPrint", "预约挂号订单趋势分析打印");
      };
    }
  },
  filters: {
