...
<template>
  ...
  >
    ...
    <el-table-column prop="overTime" label="接诊超时" show-overflow-tooltip>
      <template slot-scope="scope">
        <span style="color:#ff3939;" v-if="scope.row.eventType===3">{{ scope.row.timeOut}}</span>
      </template>
    </el-table-column>
    <el-table-column prop="overTime2" label="处方审核超时" show-overflow-tooltip>
      <template slot-scope="scope">
        <span style="color:#ff3939;" v-if="scope.row.eventType===5">{{ scope.row.timeOut}}</span>
      </template>
    </el-table-column>
    <el-table-column prop="orderNumber" label="订单编号" show-overflow-tooltip></el-table-column>
    <el-table-column prop="prescriptionId" label="处方编号" show-overflow-tooltip></el-table-column>
    <el-table-column label="订单日期" show-overflow-tooltip>
      <template slot-scope="scope">{{ scope.row.orderTime | dealTime}}</template>
    </el-table-column>
    <el-table-column label="处方日期" show-overflow-tooltip>
      <template slot-scope="scope">{{ scope.row.prescriptionTime | dealTime}}</template>
    </el-table-column>
    ...
    <el-table-column prop="doctorName" label="医护人员" show-overflow-tooltip></el-table-column>
    <el-table-column label="提醒状态" show-overflow-tooltip>
      <template slot-scope="scope">
        <span
          :class="{'color1':scope.row.remindState===1,'color0':scope.row.remindState===0}"
        >{{ scope.row.remindState | remindState}}</span>
      </template>
    </el-table-column>
    <el-table-column label="操作" fixed="right">
      <template slot-scope="scope">
        <el-button
          ...
          v-if="scope.row.eventType === 3 || scope.row.eventType === 5"
          size="mini"
          type="success"
          class="green_color_btn"
          round
        >处理</el-button>
        <el-button v-else size="mini" disabled round>处理</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
