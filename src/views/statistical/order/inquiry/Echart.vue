...
<template>
  <div class="order-echart-analysis">
    <div class="tab-scroll-container tab-scroll-container-nopagination scrollbarStyle2">
      ...
      <!-- echart柱状图 -->
      <div class="order_echart _marginTopFu" id="printEchart" v-loading="loading">
        <!-- 标题 -->
        <div
          class="_title"
        >{{searchQuery.startDate |dealTime}}至{{searchQuery.endDate|dealTime}}在线问诊订单订单趋势分析</div>
        <!-- 内容 -->
        <div class="echart-content statistical-inquiry-echart" ref="echartContent">
          <page-bar-chart class="orderEchart" ref="orderEchart" />
...
export default {
  ...
  methods: {
    ...
    initData() {
      this.init().then(res => {
        this.$refs.orderEchart.echartData = {
          legend: ["订单量(个)", "环比(%)"],
        ...
        this.$nextTick(() => {
          this.$refs.orderEchart.initChart();
        });
      });
    },
    //设置默认参数
    ...
    doPrint() {
      ...
      var cv = document.querySelector(".orderEchart canvas"); //获取canvas
      ...
      img.onload = () => {
        ...
        openPrint(this, "#printEchart", "在线问诊订单订单趋势分析打印");
      };
    }
  },
  filters: {
